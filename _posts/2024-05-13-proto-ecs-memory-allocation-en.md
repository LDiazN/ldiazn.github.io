...
There are many considerations to keep in mind when designing an entity allocator, but it essentially is a _data structure_ that:
...
- At some point, a new entity is created, and our memory allocator decides that it can use the available position **i** for this entity
...
- `free` will return the index into the free list, increasing its generation counter so that further `is_live` calls return false for this pointer
...
@TODO seguir revisando desde aqui
...
The problem with this implementation is that, due to Rust's ownership rules, now we can't use multiple mutable references at the same time using the `get_mut` function multiple times, which is a big problem when we want to implement relationships between entities. Consider the following code: 
...
If you think about it, it makes sense to provide access to many mutable references to entries within the allocator, because the **allocator couldn't care less about the current state of the values it manages**, it only cares about which entries are free, and the generation for each entry. The problem is that Rust doesn't know about this! 

Another subtle problem here is how we are creating entities, although we are [moving](https://stackoverflow.com/questions/3106110/what-is-move-semantics) the ownership of the entity to the allocator, we still need to copy its data into the array, when it could have been created within the array in the first place. This is an important consideration if we want to allocate many entities at once and we will address it later. 

# RefCell to the rescue

The borrow checker won't allow to have multiple references to a memory segment owned by the memory allocator, even though we know that the referenced segments are disjoint.

Since this is a memory allocator, what we need is:

* Many references to multiple segments of the internal array

* Mutable access to those references to update the state
...
So... yeah, not particularly comfortable to use. In this section, we will work on better usability for our entity allocators. 
...
@TODO cambiar de nombre el allocator para que se llame box allocator, creo que le da mejor estructura 
...
Also note that the pointer type we defined is big, if the generation is 32-bit long, then you have a pointer pointer with size 32 + 64 = 96 bits long!

# Final thoughts

To choose an allocator style for our system, we had to consider many properties for each one of them and how they interact with `proto-ecs`. In this section, we will list the properties of each type of allocator.
...
Now we will end this article with some simple profiling to have some idea of the performance implications of each implementation. This is not a thorough test by any means, but it helped us to form an idea to decide which implementation to use.
...
In doing so, we explored many important but non-trivial Rust features, like `MaybeUninit` and `RefCell`. We also implemented unsafe alternatives with important advantages. And although using unsafe is not particularly popular in Rust, is a language feature like any other that we can use if needed as long as we do it right. Remember, unsafe is just telling the Rust compiler "I will ensure the safety of this code on my own". Always remember to understand the safety conditions to make an unsafe code safe!